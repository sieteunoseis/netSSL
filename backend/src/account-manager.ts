      ...
      if (chainCerts.length > 0) {
        ...
        if (chainCerts.length >= 2) {
          ...
          if (fs.existsSync(isrgRootX1Path)) {
            const isrgRootX1 = await fs.promises.readFile(isrgRootX1Path, 'utf8');
            await fs.promises.writeFile(path.join(connectionEnvDir, 'root.crt'), isrgRootX1);
            Logger.info(`Using ISRG Root X1 certificate from ${isrgRootX1Path}`);
          } else {
            Logger.warn(`ISRG Root X1 certificate not found at ${isrgRootX1Path}. ISE may require this root certificate.`);
            Logger.info(`Download from: https://letsencrypt.org/certs/isrgrootx1.pem`);
          }
        }
