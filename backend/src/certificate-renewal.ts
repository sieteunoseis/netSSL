...
class CertificateRenewalServiceImpl implements CertificateRenewalService {
  ...
  async renewCertificate(
    ...
    if (operationManager) {
      const existingOperation = await operationManager.checkActiveOperation(
        connectionId,
        "certificate_renewal",
      );
      if (existingOperation) {
        throw new Error(
          "A certificate renewal is already in progress for this connection",
        );
      }
    } else {
    ...
    if (operationManager) {
      ...
      const operation = await operationManager.startOperation(
        connectionId,
        "certificate_renewal",
        "user",
        {
    ...
    Logger.info(
      `Created renewal status with ID: ${renewalId} for connection ${connectionId}`,
    );
    ...
    this.performRenewal(renewalId, connectionId, database, operationManager)
      .catch(async (error) => {
        try {
          Logger.error(
            `Certificate renewal failed for connection ${connectionId}:`,
            error,
          );
          status.status = "failed";
          status.error =
            error.message || "Unknown error during certificate renewal";
          ...
          if (database) {
            await database
              ...
              .catch((err) => {
                Logger.error(
                  `Failed to save failed renewal status to database: ${err.message}`,
                );
              });
          }
        } catch (cleanupError) {
          Logger.error(
            `Error during renewal cleanup for connection ${connectionId}:`,
            cleanupError,
          );
        }
      })
  ...
  async getRenewalStatus(renewalId: string): Promise<RenewalStatus | null> {
    ...
    if (this.database) {
      ...
      if (dbStatus) {
        ...
        const status: RenewalStatus = {
          ...
          endTime: dbStatus.updated_at
            ? new Date(dbStatus.updated_at)
            : undefined,
    ...
    Logger.info(
      `Available renewal IDs in memory: ${Array.from(this.renewalStatuses.keys()).join(", ")}`,
    );
    return null;
  }
  ...
  private async performRenewal(
    ...
    const updateStatusWithOp = (
      newStatus: RenewalStatus["status"],
      message: string,
      progress: number,
    ) =>
    ...
    try {
      ...
      Logger.info(
        `${connLabel} === Renewal started: name="${connection.name}", domain="${fullFQDN}", type="${connection.application_type}", accounts=connection-${connectionId}/ ===`,
      );
      status.logs.push(
        `Connection: id=${connectionId}, name="${connection.name}", domain="${fullFQDN}", type="${connection.application_type}"`,
      );
      ...
      await accountManager.saveRenewalLog(
        connectionId,
        fullFQDN,
        `DEBUG: About to call requestCertificate method`,
      );
      ...
      await accountManager.saveRenewalLog(
        connectionId,
        fullFQDN,
        `DEBUG: requestCertificate returned, certificate length: ${certificate ? certificate.length : "null/undefined"}`,
      );
      ...
      await accountManager.saveRenewalLog(
        connectionId,
        fullFQDN,
        `Certificate renewal completed for renewal ${renewalId}`,
      );
      ...
      await updateStatusWithOp(
        "failed",
        `Certificate renewal failed: ${errorMessage}`,
        0,
      );
  ...
  private async getRecentCertificate(
    connectionId: number,
    domain: string,
    maxAgeMs: number,
  ): Promise<string | null> {
    try {
      ...
      const connectionDir = path.join(
        process.env.ACCOUNTS_DIR || "./accounts",
      ...
      for (const certFile of certFiles) {
        ...
        if (fs.existsSync(certPath)) {
          try {
            ...
            if (fileAge <= maxAgeMs) {
              const certificateData = await fs.promises.readFile(
                certPath,
                "utf8",
              );
              const envType = isStaging ? "staging" : "production";
              Logger.info(
                `Found recent certificate for ${domain} (${Math.round(fileAge / 60000)} minutes old, ${envType}): ${certPath}`,
              );
              return certificateData;
            }

            Logger.info(
              `Certificate found but too old (${Math.round(fileAge / 60000)}min old, max ${Math.round(maxAgeMs / 60000)}min): ${certPath}`,
            );
          } catch (statError: any) {
            Logger.error(
              `Error checking file stats for ${certPath}:`,
              statError,
            );
          }
        }
      ...
      Logger.info(
        `No recent certificate found for connection ${connectionId} in ${connectionDir}`,
      );
      return null;
    } catch (error) {
      Logger.error(
        `Error checking for recent certificate for connection ${connectionId}, domain ${domain}:`,
        error,
      );
      return null;
    }
    ...
    try {
      ...
      if (!fs.existsSync(fullChainPath)) {
        ...
        if (new Date(cert.validTo) > thirtyDaysFromNow) {
          const envType = isStaging ? "staging" : "production";
          Logger.info(
            `Found existing certificate for ${domain} that is valid for more than 30 days (${envType}).`,
          );
          return certificateData;
        }
      ...
      Logger.error(
        `Error checking for existing certificate for ${domain}:`,
        error,
      );
      return null;
    }
  ...
  private async updateStatus(
    ...
    Logger.info(
      `[conn-${status.connectionId}] Renewal ${status.id}: ${message}`,
    );
    ...
    if (operationManager) {
      ...
      if (status.manualDNSEntry) {
        ...
        Logger.info(
          `Including manualDNSEntry in metadata: ${JSON.stringify(status.manualDNSEntry)}`,
        );
      }
    ...
    if (this.database) {
      await this.database
        ...
        .catch((err) => {
          Logger.error(
            `Failed to save renewal status to database: ${err.message}`,
          );
        });
    }
  }
  ...
  private async requestCertificate(
    ...
    if (sslProvider === "letsencrypt") {
      return this.requestLetsEncryptCertificate(
        connection,
        csr,
        settings,
        database,
      ...
      return this.requestZeroSSLCertificate(
        connection,
        csr,
        settings,
        status,
  ...
  private async requestLetsEncryptCertificate(
    ...
    try {
      ...
      if (csrCNOverride) {
        ...
        fullFQDN =
          primaryNode ||
          getDomainFromConnection(connection) ||
          connection.domain;
      } else {
      ...
      await accountManager.saveRenewalLog(
        connectionId,
        fullFQDN,
        `=== Let's Encrypt Certificate Renewal Started ===`,
      );
      await accountManager.saveRenewalLog(
        connectionId,
        fullFQDN,
      ...
      await accountManager.saveRenewalLog(
        connectionId,
        fullFQDN,
        `Environment: ${process.env.LETSENCRYPT_STAGING !== "false" ? "STAGING" : "PRODUCTION"}`,
      );

      await this.updateStatus(
        status,
        "creating_account",
        "Setting up Let's Encrypt account",
        20,
      );
      ...
      if (!account) {
        ...
        const email = settings.find(
          (s) => s.key_name === "LETSENCRYPT_EMAIL",
        )?.key_value;
        if (!email) {
          throw new Error(
            "Let's Encrypt email not configured in settings. Please add LETSENCRYPT_EMAIL to your settings.",
          );
        }

        Logger.info(
          `Attempting to create account as fallback with email: ${email}`,
        );
        await accountManager.saveRenewalLog(
          connectionId,
          fullFQDN,
        ...
        try {
          const newAccount = await acmeClient.createAccount(
            email,
            fullFQDN,
            connectionId,
          );
          await accountManager.saveRenewalLog(
            connectionId,
            fullFQDN,
            `Account created successfully as fallback`,
          );
          await accountManager.saveRenewalLog(
            connectionId,
            fullFQDN,
            `Account URL: ${newAccount.accountUrl}`,
          );
        } catch (accountError) {
          ...
          await accountManager.saveRenewalLog(
            connectionId,
            fullFQDN,
            `Failed to create account: ${accountError instanceof Error ? accountError.message : "Unknown error"}`,
          );
          throw new Error(
            `Failed to create Let's Encrypt account for ${fullFQDN}. ${accountError instanceof Error ? accountError.message : "Unknown error"}`,
          );
        }
      } else {
        ...
        await accountManager.saveRenewalLog(
          connectionId,
          fullFQDN,
          `Using existing account for domain: ${fullFQDN}`,
        );
        await accountManager.saveRenewalLog(
          connectionId,
          fullFQDN,
          `Account URL: ${account.accountUrl}`,
        );
      }
      ...
      await accountManager.saveRenewalLog(
        connectionId,
        fullFQDN,
        `Creating certificate order for domains: ${domains.join(", ")}`,
      );
      const order = await acmeClient.requestCertificate(csr, domains);
      await accountManager.saveRenewalLog(
        connectionId,
        fullFQDN,
        `Certificate order created: ${order.order.url}`,
      );

      await this.updateStatus(
        status,
        "creating_dns_challenge",
        "Setting up DNS challenges",
        40,
      );

      // Check DNS challenge mode and provider
      Logger.info(
        `DNS provider from connection: raw='${connection.dns_provider}', type=${typeof connection.dns_provider}`,
      );
      await accountManager.saveRenewalLog(
        connectionId,
        fullFQDN,
      ...
      await accountManager.saveRenewalLog(
        connectionId,
        fullFQDN,
        `Resolved DNS provider: '${dnsProvider}'`,
      );
      ...
      if (dnsProvider === "custom") {
        ...
        try {
          const settings =
            await this.database!.getSettingsByProvider(dnsProvider);
          ...
          switch (dnsProvider) {
            case "cloudflare":
              const cfKey = settings.find(
                (s) => s.key_name === "CF_KEY",
              )?.key_value;
              const cfZone = settings.find(
                (s) => s.key_name === "CF_ZONE",
              )?.key_value;
              hasRequiredKeys = !!(cfKey && cfZone);
              break;
            case "digitalocean":
              const doKey = settings.find(
                (s) => s.key_name === "DO_KEY",
              )?.key_value;
              hasRequiredKeys = !!doKey;
              break;
            case "route53":
              const awsKey = settings.find(
                (s) => s.key_name === "AWS_ACCESS_KEY",
              )?.key_value;
              const awsSecret = settings.find(
                (s) => s.key_name === "AWS_SECRET_KEY",
              )?.key_value;
              const awsZone = settings.find(
                (s) => s.key_name === "AWS_ZONE_ID",
              )?.key_value;
              hasRequiredKeys = !!(awsKey && awsSecret && awsZone);
              break;
            case "azure":
              const azureSub = settings.find(
                (s) => s.key_name === "AZURE_SUBSCRIPTION_ID",
              )?.key_value;
              const azureRg = settings.find(
                (s) => s.key_name === "AZURE_RESOURCE_GROUP",
              )?.key_value;
              const azureZone = settings.find(
                (s) => s.key_name === "AZURE_ZONE_NAME",
              )?.key_value;
              hasRequiredKeys = !!(azureSub && azureRg && azureZone);
              break;
            case "google":
              const googleProject = settings.find(
                (s) => s.key_name === "GOOGLE_PROJECT_ID",
              )?.key_value;
              const googleZone = settings.find(
                (s) => s.key_name === "GOOGLE_ZONE_NAME",
              )?.key_value;
          ...
          if (!hasRequiredKeys) {
            ...
            await accountManager.saveRenewalLog(
              connectionId,
              fullFQDN,
              `API keys not configured for ${dnsProvider} provider, falling back to manual DNS`,
            );
            status.logs.push(
              `API keys not configured for ${dnsProvider} provider, using manual DNS mode`,
            );
          }
        } catch (error) {
          ...
          await accountManager.saveRenewalLog(
            connectionId,
            fullFQDN,
            `Failed to check API keys for ${dnsProvider} provider: ${error instanceof Error ? error.message : "Unknown error"}`,
          );
      ...
      if (forceManualDNS) {
        ...
        await accountManager.saveRenewalLog(
          connectionId,
          fullFQDN,
          `Manual DNS challenge required (${reason}) for ${dnsProvider} provider`,
        );
        ...
        await this.updateStatus(
          status,
          "completing_validation",
          "Completing Let's Encrypt validation",
          70,
        );
        ...
        const certificateData = await acmeClient.finalizeCertificate(
          order.order,
          csr,
        );
        await accountManager.saveRenewalLog(
          connectionId,
          fullFQDN,
          "Let's Encrypt certificate downloaded successfully",
        );
        ...
        await accountManager.saveRenewalLog(
          connectionId,
          fullFQDN,
          `DEBUG: About to save certificate chain. Certificate data length: ${certificateData.length}`,
        );
        Logger.debug(
          `About to save certificate chain for ${fullFQDN}. Certificate data length: ${certificateData.length}`,
        );

        if (!certificateData || certificateData.length === 0) {
          throw new Error(
            "Certificate data is empty - cannot save certificate chain",
          );
        }
      ...
      try {
        ...
        switch (dnsProvider) {
          ...
          case "route53": {
            const { Route53DNSProvider } =
              await import("./dns-providers/route53");
            provider = await Route53DNSProvider.create(database, fullFQDN);
            break;
          }
          case "digitalocean": {
            const { DigitalOceanDNSProvider } =
              await import("./dns-providers/digitalocean");
          ...
          case "google": {
            const { GoogleDNSProvider } =
              await import("./dns-providers/google");
            provider = await GoogleDNSProvider.create(database, fullFQDN);
            break;
          }
          default:
            throw new Error(
              `Unsupported DNS provider for automated challenges: ${dnsProvider}`,
            );
        }
        await accountManager.saveRenewalLog(
          connectionId,
          fullFQDN,
          `${dnsProvider} DNS provider initialized`,
        );
        ...
        await accountManager.saveRenewalLog(
          connectionId,
          fullFQDN,
          `Setting up ${order.challenges.length} DNS challenge(s)`,
        );
        ...
        for (const challenge of order.challenges) {
          ...
          await accountManager.saveRenewalLog(
            connectionId,
            fullFQDN,
            `Processing challenge for domain: ${challengeDomain}`,
          );
          await accountManager.saveRenewalLog(
            connectionId,
            fullFQDN,
          ...
          if (
            ...
            typeof provider.cleanupTxtRecords === "function"
          ) {
            await accountManager.saveRenewalLog(
              connectionId,
              fullFQDN,
              `Cleaning up existing TXT records for ${challengeDomain}`,
            );
          ...
          await accountManager.saveRenewalLog(
            connectionId,
            fullFQDN,
            `Created DNS TXT record for ${challengeDomain}: ${record.id}`,
          );
          status.logs.push(
            `Created DNS TXT record for ${challengeDomain}: ${record.id}`,
          );
        ...
        await accountManager.saveRenewalLog(
          connectionId,
          fullFQDN,
          `Waiting for DNS propagation of ${dnsRecords.length} record(s)`,
        );
        ...
        for (const { challenge, domain } of dnsRecords) {
          ...
          await accountManager.saveRenewalLog(
            connectionId,
            fullFQDN,
            `Verifying DNS propagation for ${domain}, expected value: ${expectedValue}`,
          );
          ...
          if (
            dnsProvider === "cloudflare" &&
            typeof provider.verifyTxtRecord === "function"
          ) {
            isVerified = await provider.verifyTxtRecord(domain, expectedValue);
          } else {
            isVerified = await provider.waitForDNSPropagation(
              acmeRecordName,
              expectedValue,
            );
          }
          ...
          if (!isVerified) {
            await accountManager.saveRenewalLog(
              connectionId,
              fullFQDN,
              `ERROR: DNS propagation verification failed for ${domain}`,
            );
            throw new Error(
              `DNS propagation verification failed for ${domain}`,
            );
          }
          ...
          await accountManager.saveRenewalLog(
            connectionId,
            fullFQDN,
            `DNS propagation verified for ${domain}`,
          );
        ...
        await accountManager.saveRenewalLog(
          connectionId,
          fullFQDN,
          `Completing ${dnsRecords.length} Let's Encrypt challenge(s)`,
        );
        ...
        for (const { challenge } of dnsRecords) {
          await accountManager.saveRenewalLog(
            connectionId,
            fullFQDN,
            `Completing challenge: ${challenge.url}`,
          );
          await acmeClient.completeChallenge(challenge);
          await accountManager.saveRenewalLog(
            connectionId,
            fullFQDN,
            `Challenge completed successfully: ${challenge.url}`,
          );
        }

        // Add delay to ensure Let's Encrypt processes the completed challenges
        Logger.info(
          "Waiting for Let's Encrypt to process challenge completion...",
        );
        await accountManager.saveRenewalLog(
          connectionId,
          fullFQDN,
          `Waiting 3 seconds for Let's Encrypt to process challenge completion...`,
        );
        ...
        await accountManager.saveRenewalLog(
          connectionId,
          fullFQDN,
          `Waiting for order completion: ${order.order.url}`,
        );
        const completedOrder = await acmeClient.waitForOrderCompletion(
          order.order,
        );
        await accountManager.saveRenewalLog(
          connectionId,
        ...
        const certificate = await acmeClient.finalizeCertificate(
          completedOrder,
          csr,
        );
        await accountManager.saveRenewalLog(
        ...
        if (!isStaging || process.env.LETSENCRYPT_CLEANUP_DNS === "true") {
          await accountManager.saveRenewalLog(
            connectionId,
            fullFQDN,
            `Cleaning up ${dnsRecords.length} DNS TXT record(s)`,
          );
          for (const { record } of dnsRecords) {
            try {
              ...
              if (
                dnsProvider === "cloudflare" &&
                typeof provider.deleteTxtRecord === "function"
              ) {
              ...
              await accountManager.saveRenewalLog(
                connectionId,
                fullFQDN,
                `Cleaned up DNS TXT record: ${record.id}`,
              );
              ...
              await accountManager.saveRenewalLog(
                connectionId,
                fullFQDN,
                `WARNING: Failed to clean up DNS record ${record.id}: ${error instanceof Error ? error.message : "Unknown error"}`,
              );
            }
          }
        } else {
          Logger.info(
            `Skipping DNS record cleanup in staging mode (${dnsRecords.length} records)`,
          );
          await accountManager.saveRenewalLog(
            connectionId,
            fullFQDN,
        ...
        await accountManager.saveRenewalLog(
          connectionId,
          fullFQDN,
          `DEBUG: About to save certificate chain. Certificate data length: ${certificate.length}`,
        );
        Logger.debug(
          `About to save certificate chain for ${fullFQDN}. Certificate data length: ${certificate.length}`,
        );
        ...
        await accountManager.saveRenewalLog(
          connectionId,
          fullFQDN,
          `Automated DNS failed for ${dnsProvider}: ${errorMessage}`,
        );
      ...
      try {
        ...
        if (errorObj && typeof errorObj === "object") {
          await accountManager.saveRenewalLog(
            connectionId,
            fullFQDN,
            `Detailed error: ${JSON.stringify(errorObj, null, 2)}`,
          );

          // If we have authorization URLs, try to get detailed auth info
          if (
            errorObj.authorizations &&
            Array.isArray(errorObj.authorizations)
          ) {
            await accountManager.saveRenewalLog(
              connectionId,
              fullFQDN,
              `Attempting to fetch detailed authorization information...`,
            );
  ...
  private async requestZeroSSLCertificate(
    connection: ConnectionRecord,
    csr: string,
    _settings: any[],
    status: RenewalStatus,
    ...
    try {
      ...
      await this.updateStatus(
        status,
        "requesting_certificate",
        "Creating ZeroSSL certificate request",
        30,
      );
      ...
      status.logs.push(
        `ZeroSSL certificate created with ID: ${certificate.id}`,
      );
      ...
      status.logs.push(
        `Retrieved validation details for ${validations.length} domains`,
      );
      ...
      await this.updateStatus(
        status,
        "waiting_dns_propagation",
        "Waiting for DNS propagation via MXToolbox",
        50,
      );
      ...
      const certificateData = await zeroSSL.waitForCertificate(
        certificate.id,
        300000,
      ); // 5 minutes
  ...
  private async createDNSRecordForValidation(
    dnsProvider: string,
    recordName: string,
    recordValue: string,
    ...
    try {
      if (dnsProvider === "cloudflare") {
        ...
        const { DigitalOceanDNSProvider } =
          await import("./dns-providers/digitalocean");
        const digitalOcean = await DigitalOceanDNSProvider.create(
          this.database,
          recordName,
        );
        ...
        const route53 = await Route53DNSProvider.create(
          this.database,
          recordName,
        );
        ...
        const google = await GoogleDNSProvider.create(
          this.database,
          recordName,
        );
        ...
        throw new Error(
          `DNS provider ${dnsProvider} not supported for ZeroSSL CNAME validation`,
        );
      }
    } catch (error) {
      Logger.error(
  ...
  private async handleDNSChallenge(
    connectionId: number,
    connection: ConnectionRecord,
    database: DatabaseManager,
    status: RenewalStatus,
  ...
  private async handleDigitalOceanChallenge(
    connection: ConnectionRecord,
    _settings: any[],
    status: RenewalStatus,
  ): Promise<void> {
    ...
    try {
      ...
      const digitalOceanDNS = await DigitalOceanDNSProvider.create(
        this.database,
        fullFQDN,
      );
      ...
      await this.updateStatus(
        status,
        "dns_validation",
        "Creating DNS records in DigitalOcean",
        50,
      );
      ...
      for (const authz of this.authzRecords) {
        ...
        const record = await digitalOceanDNS.createDNSRecord(
          recordName,
          keyAuth,
          "TXT",
        );
        status.logs.push(
          `Created DigitalOcean DNS record with ID: ${record.id}`,
        );
      ...
      await this.updateStatus(
        status,
        "completing_validation",
        "DNS propagation confirmed - completing validation",
        70,
      );
    } catch (error) {
      Logger.error(
        `DigitalOcean DNS challenge failed: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
      throw error;
    }
  }

  private async handleRoute53Challenge(
    connection: ConnectionRecord,
    _settings: any[],
    status: RenewalStatus,
  ): Promise<void> {
    ...
    try {
      ...
      const route53DNS = await Route53DNSProvider.create(
        this.database,
        fullFQDN,
      );
      ...
      await this.updateStatus(
        status,
        "dns_validation",
        "Creating DNS records in Route53",
        50,
      );
      ...
      for (const authz of this.authzRecords) {
        ...
        // Create the DNS record
        const record = await route53DNS.createDNSRecord(
          recordName,
          keyAuth,
          "TXT",
        );
      ...
      Logger.error(
        `Route53 DNS challenge failed: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
      throw error;
    }
  }

  private async handleAzureChallenge(
    connection: ConnectionRecord,
    _settings: any[],
    status: RenewalStatus,
  ): Promise<void> {
    ...
    try {
      ...
      for (const authz of this.authzRecords) {
        ...
        // Create the DNS record
        const record = await azureDNS.createDNSRecord(
          recordName,
          keyAuth,
          "TXT",
        );
      ...
      Logger.error(
        `Azure DNS challenge failed: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
      throw error;
    }
  }

  private async handleGoogleChallenge(
    connection: ConnectionRecord,
    _settings: any[],
    status: RenewalStatus,
  ): Promise<void> {
    ...
    try {
      ...
      await this.updateStatus(
        status,
        "dns_validation",
        "Creating DNS records in Google Cloud",
        50,
      );
      ...
      for (const authz of this.authzRecords) {
        ...
        // Create the DNS record
        const record = await googleDNS.createDNSRecord(
          recordName,
          keyAuth,
          "TXT",
        );
      ...
      Logger.error(
        `Google Cloud DNS challenge failed: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
      throw error;
    }
  }

  private async handleCustomDNSChallenge(
    connectionId: number,
    connection: ConnectionRecord,
    _settings: any[],
    status: RenewalStatus,
    ...
    try {
      ...
      await this.updateStatus(
        status,
        "waiting_manual_dns",
        "Manual DNS entry required - waiting for admin",
        60,
        operationManager,
      );
      ...
      for (const challenge of challenges) {
        ...
        const instructions = customDNS.getManualInstructions(
          recordName,
          keyAuth,
        );
        await accountManager.saveRenewalLog(
        ...
        status.logs.push(
          "Manual DNS entry required - check renewal logs for instructions",
        );
        ...
        Logger.info(
          `About to update status to waiting_manual_dns with manualDNSEntry: ${JSON.stringify(status.manualDNSEntry)}`,
        );
        await this.updateStatus(
          status,
          "waiting_manual_dns",
        ...
        await accountManager.saveRenewalLog(
          connectionId,
          fullFQDN,
          `Waiting for manual DNS entry. Timeout: ${maxWaitTime / 1000} seconds`,
        );

        const isVerified = await customDNS.waitForManualEntry(
          recordName,
          keyAuth,
          maxWaitTime,
          () => this.isCancelled(status.id),
        );

        if (!isVerified) {
          throw new Error(
            `Manual DNS entry verification timed out after ${maxWaitTime / 1000} seconds`,
          );
        }
        ...
        await this.updateStatus(
          status,
          "completing_validation",
          "DNS record verified - completing validation",
          60,
          operationManager,
        );
        ...
        await accountManager.saveRenewalLog(
          connectionId,
          fullFQDN,
          `Manual DNS entry verified successfully`,
        );
      ...
      await this.updateStatus(
        status,
        "completing_validation",
        "Manual DNS entries verified - completing validation",
        70,
        operationManager,
      );
    } catch (error) {
      Logger.error(
        `Custom DNS challenge failed: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
      throw error;
    }
  }

  private async saveCertificateChain(
    connectionId: number,
    domain: string,
    ...
    try {
      await accountManager.saveRenewalLog(
        connectionId,
        domain,
        `DEBUG: saveCertificateChain called with domain: ${domain}, certificateData length: ${certificateData.length}`,
      );
      Logger.debug(
        `saveCertificateChain called with domain: ${domain}, certificateData length: ${certificateData.length}`,
      );
      ...
      try {
        ...
        if (this.database) {
          ...
          const connection = connections.find(
            (conn) => conn.id === Number(connectionId),
          );
          if (connection && connection.ise_private_key) {
            ...
            await accountManager.saveRenewalLog(
              connectionId,
              domain,
              `DEBUG: Found ISE private key in database, length: ${privateKey.length}`,
            );
            ...
            await accountManager.saveRenewalLog(
              connectionId,
              domain,
              `DEBUG: Found general private key in database, length: ${privateKey.length}`,
            );
          }
        }
        ...
        if (!privateKey) {
          const existingCert = await accountManager.loadCertificate(
            connectionId,
            domain,
          );
          if (existingCert) {
            ...
            await accountManager.saveRenewalLog(
              connectionId,
              domain,
              `DEBUG: Found existing private key in files, length: ${privateKey.length}`,
            );
          }
        }
      } catch (error) {
        ...
        Logger.debug(
          `No private key found for ${domain}, this is normal for VOS applications`,
        );
        await accountManager.saveRenewalLog(
          connectionId,
          domain,
          `DEBUG: No private key found for ${domain}`,
        );
      }
      ...
      await accountManager.saveRenewalLog(
        connectionId,
        domain,
        `Saving certificate chain for ${domain}`,
      );
      await accountManager.saveRenewalLog(
        connectionId,
        domain,
        `DEBUG: About to call accountManager.saveCertificateChain`,
      );
      await accountManager.saveCertificateChain(
        connectionId,
        domain,
        certificateData,
        privateKey,
      ...
      await accountManager.saveRenewalLog(
        connectionId,
        domain,
        `DEBUG: accountManager.saveCertificateChain completed successfully`,
      );
      ...
      await accountManager.saveRenewalLog(
        connectionId,
        domain,
        `Saved certificate files for ${domain}`,
      );
      await accountManager.saveRenewalLog(
        connectionId,
        domain,
        `- Domain certificate: certificate.pem, ${domain}.crt`,
      );
      await accountManager.saveRenewalLog(
        connectionId,
        domain,
      ...
      if (privateKey) {
        await accountManager.saveRenewalLog(
          connectionId,
          domain,
          `- Private key: private_key.pem, ${domain}.key`,
        );
      }
  ...
  private async updateDatabaseWithRenewal(
    connectionId: number,
    database: DatabaseManager,
  ): Promise<void> {
    try {
      ...
      Logger.info(
        `Updated database with renewal info for connection ${connectionId}`,
      );
    } catch (error) {
      Logger.error(
        `Failed to update database with renewal info for connection ${connectionId}:`,
        error,
      );
      throw error;
    }
