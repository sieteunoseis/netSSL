...
export const validateConnectionData = (
  data: any,
  ...
  if (data.application_type === "vos") {
    ...
    if (!data.hostname || typeof data.hostname !== "string") {
      ...
      if (!hostnamePattern.test(data.hostname)) {
        errors.push(
          "Hostname must be a valid hostname (letters, numbers, hyphens only)",
        );
      }
    }
    ...
    // ISE: hostname is the PAN identifier — can be a short hostname or FQDN.
    // The actual FQDN for certificates comes from the ise_nodes field.
    if (data.hostname !== undefined && typeof data.hostname !== "string") {
      ...
      errors.push("ISE Admin Node must contain only ASCII characters");
    }
    ...
    if (data.hostname !== undefined && typeof data.hostname !== "string") {
      ...
      const flexibleHostnamePattern =
        /^(\*|[a-zA-Z0-9]([a-zA-Z0-9\-]{0,61}[a-zA-Z0-9])?)?$/;
      if (!flexibleHostnamePattern.test(data.hostname)) {
        errors.push(
          "Hostname must be a valid hostname, wildcard (*), or blank",
        );
      }
    }
  }
  ...
  const requiresCredentials =
    data.application_type === "vos" ||
    data.application_type === "catalyst_center" ||
  ...
  if (requiresCredentials) {
    ...
    if (
      data.ise_certificate &&
      typeof data.ise_certificate === "string" &&
      data.ise_certificate.trim() !== ""
    ) {
      if (
        !data.ise_certificate.includes("-----BEGIN CERTIFICATE REQUEST-----")
      ) {
    ...
    if (
      data.ise_private_key &&
      typeof data.ise_private_key === "string" &&
      data.ise_private_key.trim() !== ""
    ) {
      if (
        !data.ise_private_key.includes("-----BEGIN PRIVATE KEY-----") &&
        !data.ise_private_key.includes("-----BEGIN RSA PRIVATE KEY-----")
      ) {
    ...
    if (
      data.ise_cert_import_config &&
      typeof data.ise_cert_import_config === "string" &&
      data.ise_cert_import_config.trim() !== ""
    ) {
      try {
        ...
        if (typeof config !== "object" || Array.isArray(config)) {
          errors.push(
            "ISE Certificate Import Configuration must be a valid JSON object",
          );
        }
      } catch (e) {
  ...
  if (
    ...
    if (typeof data.version !== "string") {
      ...
      !validator.isDecimal(data.version, {
        force_decimal: false,
        decimal_digits: "1,2",
        locale: "en-US",
      })
      ...
      errors.push(
        "Version must be a valid decimal number (e.g., 1.0, 2.5, 10.15)",
      );
    }
  }
  ...
  if (
    ...
    if (
      !validator.isIn(data.application_type, [
        "vos",
        "ise",
        "general",
        "catalyst_center",
      ...
      errors.push(
        'Application type must be one of "vos", "ise", "general", or "catalyst_center"',
      );
    }
  }
  ...
  if (
    data.ise_application_subtype !== undefined &&
    data.ise_application_subtype !== null &&
    data.ise_application_subtype !== ""
  ) {
    if (
      !validator.isIn(data.ise_application_subtype, [
        "multi_use",
        "admin",
        "eap",
        "dtls",
      ...
      errors.push(
        'ISE certificate usage must be one of "multi_use", "admin", "eap", "dtls", "guest", "portal", "pxgrid", "saml", or "ims"',
      );
    }
  }
  ...
  if (
    data.ise_csr_config !== undefined &&
    data.ise_csr_config !== null &&
    data.ise_csr_config !== ""
  ) {
    if (
      typeof data.ise_csr_config === "string" &&
      data.ise_csr_config.trim() !== ""
    ) {
      try {
  ...
  if (
    data.custom_csr !== undefined &&
    data.custom_csr !== null &&
    data.custom_csr !== ""
  ) {
    if (typeof data.custom_csr !== "string") {
      errors.push("Custom CSR must be a string");
    } else if (
      !data.custom_csr.includes("-----BEGIN CERTIFICATE REQUEST-----") ||
      !data.custom_csr.includes("-----END CERTIFICATE REQUEST-----")
    ) {
      errors.push(
        "Custom CSR must be a valid PEM formatted certificate request",
      );
    }
  }
  ...
  if (data.application_type === "general") {
    if (data.general_private_key && data.general_private_key.trim() !== "") {
      if (
        !data.general_private_key.includes("-----BEGIN PRIVATE KEY-----") &&
        !data.general_private_key.includes("-----BEGIN RSA PRIVATE KEY-----")
      ) {
        errors.push("Private key must be in PEM format");
      }
    }
    ...
    if (data.enable_ssh) {
      ...
      if (
        data.ssh_cert_path &&
        typeof data.ssh_cert_path === "string" &&
        !data.ssh_cert_path.startsWith("/")
      ) {
        errors.push(
          "Remote certificate path must be an absolute path (starting with /)",
        );
      }
      if (
        data.ssh_key_path &&
        typeof data.ssh_key_path === "string" &&
        !data.ssh_key_path.startsWith("/")
      ) {
        errors.push(
          "Remote private key path must be an absolute path (starting with /)",
        );
      }
      if (
        data.ssh_chain_path &&
        typeof data.ssh_chain_path === "string" &&
        !data.ssh_chain_path.startsWith("/")
      ) {
        errors.push(
          "Remote chain path must be an absolute path (starting with /)",
        );
      }
    }
  }
  ...
  if (
    data.cc_list_of_users !== undefined &&
    data.cc_list_of_users !== null &&
    data.cc_list_of_users !== ""
  ) {
    if (
      !validator.isIn(data.cc_list_of_users, [
        "server",
        "ipsec",
        "server,ipsec",
      ])
      ...
      errors.push(
        'Certificate usage must be one of "server", "ipsec", or "server,ipsec"',
      );
    }
  }
  ...
  if (
    ...
    if (typeof data.alt_names !== "string") {
      ...
      const altNames = data.alt_names
        .split(",")
        .map((name: string) => name.trim());
      for (const altName of altNames) {
        if (
          altName &&
          !validator.isFQDN(altName, { allow_numeric_tld: true })
        ) {
...
export const sanitizeConnectionData = (
  ...
  const sanitized: Partial<ConnectionRecord> = {
    ...
    application_type: (["vos", "ise", "general", "catalyst_center"].includes(
      data.application_type,
    )
      ? data.application_type
      : "vos") as "vos" | "ise" | "general" | "catalyst_center",
    ...
    ise_csr_config: data.ise_csr_config
      ? String(data.ise_csr_config)
      : undefined, // Don't escape JSON content
    ...
    general_private_key: data.general_private_key
      ? String(data.general_private_key)
      : undefined, // Don't escape private key content
    ise_nodes: data.ise_nodes
      ? validator.escape(String(data.ise_nodes))
      : undefined,
    ise_certificate: data.ise_certificate
      ? String(data.ise_certificate)
      : undefined, // Don't escape certificate content
    ise_private_key: data.ise_private_key
      ? String(data.ise_private_key)
      : undefined, // Don't escape private key content
    ise_cert_import_config: data.ise_cert_import_config
      ? String(data.ise_cert_import_config)
      : undefined, // Don't escape JSON content
    cc_list_of_users: ["server", "ipsec", "server,ipsec"].includes(
      data.cc_list_of_users,
    )
      ? data.cc_list_of_users
      : undefined,
  ...
  if (data.auto_renew_status !== undefined) {
    sanitized.auto_renew_status = validator.escape(
      String(data.auto_renew_status || ""),
    );
  }

  if (data.auto_renew_last_attempt !== undefined) {
    sanitized.auto_renew_last_attempt = validator.escape(
      String(data.auto_renew_last_attempt || ""),
    );
  }
