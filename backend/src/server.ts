...
// Check for ISE connections with the same cert name targeting overlapping nodes.
// Returns warning strings (non-blocking) so the user can differentiate certs.
async function checkISECertNameConflicts(
  ...
  let certName = "";
  try {
    const config = JSON.parse(iseCertImportConfig);
    certName = config.name || "";
  } catch {
    return warnings;
  }
  if (!certName) return warnings;

  const nodes = iseNodes
    .split(",")
    .map((n) => n.trim())
    .filter(Boolean);
  ...
  try {
    const allConnections = await database.getAllConnections();
    for (const conn of allConnections) {
      if (conn.id === connectionId) continue;
      if (conn.application_type !== "ise") continue;
      if (!conn.ise_nodes || !conn.ise_cert_import_config) continue;

      let otherName = "";
      try {
        const otherConfig = JSON.parse(conn.ise_cert_import_config);
        otherName = otherConfig.name || "";
      } catch {
        continue;
      }

      if (otherName !== certName) continue;

      const otherNodes = conn.ise_nodes
        .split(",")
        .map((n: string) => n.trim())
        .filter(Boolean);
      const overlap = nodes.filter((n) => otherNodes.includes(n));
      if (overlap.length > 0) {
        warnings.push(
          `Certificate name "${certName}" is also used by connection "${conn.name}" (ID ${conn.id}) ` +
    ...
    Logger.warn("Error checking ISE cert name conflicts:", error);
  }
  return warnings;
}
...
app.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const {
      password: _pw,
      ise_private_key: _ipk,
      general_private_key: _gpk,
      ...safeBody
    } = req.body;
    Logger.info(
    ...
    if (sanitizedData.application_type === "ise" && sanitizedData.ise_nodes) {
      const primaryNode = sanitizedData.ise_nodes
        .split(",")
        .map((n: string) => n.trim())
        .filter((n: string) => n)[0];
      if (primaryNode && primaryNode.includes(".")) {
        sanitizedData.domain = primaryNode.substring(
          primaryNode.indexOf(".") + 1,
        );
      }
    }
    ...
    const connectionId = await database.createConnection(
      sanitizedData as ConnectionRecord,
    );
    ...
    if (sanitizedData.ssl_provider === "letsencrypt") {
      try {
        ...
        const domain =
          getDomainFromConnection(sanitizedData as ConnectionRecord) || "";

        // Check if account already exists
        const existingAccount = await acmeClient.loadAccount(
          domain,
          connectionId,
        );
        if (!existingAccount) {
          Logger.info(
            `Pre-creating Let's Encrypt account for new connection: ${domain}`,
          );
          const sslSettings =
            await database.getSettingsByProvider("letsencrypt");
          const email = sslSettings.find(
          ...
          if (email) {
            ...
            acmeClient
              .createAccount(email, domain, connectionId)
              .then(() => {
                Logger.info(
                  `Successfully pre-created Let's Encrypt account for ${domain}`,
                );
              })
              .catch((error) => {
                Logger.error(
                  `Failed to pre-create Let's Encrypt account for ${domain}:`,
                  error,
                );
    ...
    // Check for ISE cert name conflicts with other connections
    const warnings = await checkISECertNameConflicts(
      connectionId,
      req.body.ise_nodes,
...
app.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!newName || !password) {
      return res
        .status(400)
        .json({ error: "newName and password are required" });
    }
...
app.put(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const validation = validateConnectionData(req.body);
    if (!validation.isValid) {
      Logger.warn("Validation failed for connection update:", {
        id,
    ...
    if (sanitizedData.ssl_provider === "letsencrypt") {
      try {
        ...
        if (!existingAccount) {
          Logger.info(
            `Pre-creating Let's Encrypt account for updated connection: ${domain}`,
          );
          const sslSettings =
            await database.getSettingsByProvider("letsencrypt");
          const email = sslSettings.find(
        ...
        Logger.error(
          "Error during Let's Encrypt account pre-creation on update:",
          error,
        );
...
app.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
      ...
      const renewalStatus = await certificateRenewalService.renewCertificate(
        id,
        database,
        operationManager,
      );
      ...
      Logger.error(
        `Error starting certificate renewal for connection ${id}: ${error.message}`,
      );
...
app.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (connection.application_type !== "ise") {
      return res.status(400).json({
        error: "Connection must be of type ISE for certificate import",
      });
    }
    ...
    if (!connection.username || !connection.password) {
      return res.status(400).json({
        error: "ISE username and password are required for certificate import",
      });
    }
    ...
    if (!connection.ise_nodes) {
      return res
        .status(400)
        .json({ error: "ISE nodes must be configured for certificate import" });
    }
    ...
    if (!certificateData || !privateKeyData) {
      return res
        .status(400)
        .json({ error: "Certificate data and private key data are required" });
    }
    ...
    try {
      ...
      if (connection.ise_cert_import_config) {
        try {
          ...
          Logger.warn(
            `Invalid JSON in ise_cert_import_config for connection ${id}, using defaults`,
          );
        }
      }
      ...
      // Parse ISE nodes
      const nodes = connection.ise_nodes
        .split(",")
        .map((node) => node.trim())
        .filter((node) => node);
      ...
      if (
        ...
        for (const node of nodes) {
          try {
            ...
            if (!caResult.success) {
              Logger.warn(
                `Failed to upload CA certificates to ${node}: ${caResult.message}`,
              );
            } else {
      ...
      Logger.error(
        `Error during ISE certificate import for connection ${id}:`,
        error,
      );
      return res.status(500).json({
...
app.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!hostname || !username || !password) {
      return res
        .status(400)
        .json({ error: "Hostname, username, and password are required" });
    }

    try {
      const iseProvider = PlatformFactory.createProvider("ise") as ISEProvider;
      const nodes = await iseProvider.getDeploymentNodes(
        hostname,
        username,
        password,
      );
      ...
      Logger.error(
        `Error fetching ISE deployment nodes from ${hostname}:`,
        error,
      );
...
app.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
      const status =
        await certificateRenewalService.getRenewalStatus(renewalId);
      if (!status) {
      ...
      const serializedStatus = {
        ...status,
        startTime:
          status.startTime instanceof Date
            ? status.startTime.toISOString()
            : status.startTime,
        endTime: status.endTime
          ? status.endTime instanceof Date
    ...
    try {
      ...
      const operationsWithAge = operations.map((op) => ({
        ...
        ageMinutes: Math.floor(
          (Date.now() - new Date(op.started_at).getTime()) / 1000 / 60,
        ),
      }));
...
app.post(
  "/api/operations/:operationId/force-complete",
  asyncHandler(async (req: Request, res: Response) => {
    const operationId = req.params.operationId;
    const { status = "failed", error = "Operation manually cancelled" } =
      req.body;
    ...
    try {
      ...
      await operationManager.updateOperation(operationId, {
        ...
        message:
          status === "completed"
            ? "Operation completed manually"
            : "Operation cancelled manually",
      });
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (forceStuck) {
        ...
        const stuckOperations = stuckOps.filter(
          (op) =>
        ...
        Logger.info(
          `Force completed ${stuckOperations.length} stuck operations`,
        );
      }
...
app.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
      ...
      const filePath = accountManager.getCertificateFilePath(
        connectionId,
        baseFilename,
      );
      ...
      if (!fs.existsSync(filePath)) {
        return res
          .status(404)
          .json({ error: `Certificate file not found: ${certType}` });
      }
      ...
      Logger.error(
        `Error downloading certificate ${certType} for connection ${connectionId}:`,
        error,
      );
      return res.status(500).json({
...
app.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    Logger.debug(
      `Certificate listing for connection ${connectionId} (${domain})`,
    );
      ...
      const renewalSettings = [
        {
          ...
          provider: "renewal",
          description:
            "Number of days before certificate expiration to automatically renew",
        },
        {
          ...
          provider: "renewal",
          description:
            "Number of days before expiration to display warning in UI",
        },
        {
...
app.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!key_name || !key_value || !provider) {
      return res
        .status(400)
        .json({ error: "key_name, key_value, and provider are required" });
    }
    ...
    if (envOnlySettings.includes(key_name)) {
      return res
        .status(400)
        .json({ error: `${key_name} must be configured in the .env file` });
    }
...
app.delete(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (envOnlySettings.includes(keyName)) {
      return res
        .status(400)
        .json({ error: `${keyName} must be configured in the .env file` });
    }
...
app.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const getKey = (name: string) =>
      settings.find((s) => s.key_name === name)?.key_value;

    Logger.info(
      `Testing provider: ${provider}, found ${settings.length} settings: ${settings.map((s) => s.key_name).join(", ")}`,
    );
      ...
      switch (provider) {
        ...
        case "digitalocean": {
          ...
          if (doResponse.ok) {
            ...
            return res.json({
              success: true,
              message: `Connected as: ${doData.account?.email || "verified"}`,
            });
          }
          return res.json({
            success: false,
            message: `HTTP ${doResponse.status}: Authentication failed`,
          });
        }
        case "route53": {
          ...
          return res.json({
            success: true,
            message: `Credentials present for zone ${zoneId}`,
          });
        }
        case "letsencrypt": {
          ...
          if (!email) {
            return res.json({
              success: false,
              message: "Missing LETSENCRYPT_EMAIL",
            });
          }
        ...
        case "zerossl": {
          ...
          return res.json({
            success: true,
            message: "ZeroSSL API key present",
          });
        }
        case "azure": {
          ...
          return res.json({
            success: true,
            message: `Credentials present for zone ${zone}`,
          });
        }
        case "google": {
          ...
          if (!projectId || !zoneName) {
            return res.json({
              success: false,
              message: "Missing required Google Cloud credentials",
            });
          }
          return res.json({
            success: true,
            message: `Credentials present for project ${projectId}`,
          });
        }
        case "custom": {
          ...
          if (!dns1) {
            return res.json({
              success: false,
              message: "Missing CUSTOM_DNS_SERVER_1",
            });
          }
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!connection.is_enabled) {
      return res.status(200).json({
        ...
        details:
          "Certificate information not available for disabled connections",
      });
    }
    ...
    const certInfo = await getCertificateInfoWithFallback(
      domain,
      connection,
      database,
    );
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (!domain) {
        return res
          .status(400)
          .json({ error: "Invalid domain configuration for this connection" });
      }
...
app.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
      ...
      const metrics = await database.getCertificateMetrics(
        connectionId,
        limit,
        hours,
      );
      const averageMetrics = await database.getAverageMetrics(
        connectionId,
        hours,
      );
...
app.get(
  "/api/auto-renewal/status",
  asyncHandler(async (req: Request, res: Response) => {
    try {
      const connections = await database.getAllConnections();

      // Filter to only connections with auto-renewal enabled
      const autoRenewConnections = connections.filter(
        (conn) =>
      ...
      const renewalThresholdDays = parseInt(
        process.env.CERT_RENEWAL_DAYS || "7",
      );
      ...
      for (const connection of autoRenewConnections) {
        try {
          ...
          if (
            ...
            certInfo.daysUntilExpiry <= renewalThresholdDays
          ) {
          ...
          Logger.debug(
            `Could not check certificate info for ${getDomainFromConnection(connection) || "unknown"}: ${error}`,
          );
        }
      }
      ...
      return res
        .status(500)
        .json({ error: "Failed to get auto-renewal status" });
    }
  }),
);
    ...
    try {
      ...
      const logsData = await Promise.all(
        connections.map(async (connection) => {
          ...
          try {
            const logs = await accountManager.getRenewalLog(
              connection.id!,
              domain,
            );
            return {
...
app.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    Logger.info(
      `Testing SSH connection to ${hostname} (type: ${application_type || "vos"})`,
    );

    try {
      const result =
        application_type === "general"
          ? await SSHClient.testGenericConnection({
              hostname,
              username,
              password,
            })
    ...
    try {
      const result =
        connection.application_type === "general"
          ...
          : await SSHClient.testConnection({
              hostname: fqdn,
              username: connection.username,
              password: connection.password,
            });
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (connection.application_type !== "vos" && !connection.application_type) {
      return res.status(400).json({
        ...
        details:
          "This endpoint is designed for Cisco VOS applications (CUCM, CUC, CER)",
      });
    }
    ...
    try {
      ...
      Logger.error(
        `Error starting service restart operation for ${fqdn}: ${error.message}`,
      );
      return res.status(500).json({
...
async function performServiceRestart(
  operationId: string,
  connection: any,
  fqdn: string,
) {
  ...
  try {
    ...
    if (domain) {
      await accountManager.saveRenewalLog(
        connection.id,
        domain,
        `Service restart initiated for ${fqdn}`,
      );
    }
    ...
    if (domain) {
      await accountManager.saveRenewalLog(
        connection.id,
        domain,
        `Testing SSH connection to ${fqdn}...`,
      );
    }
    ...
    if (!sshTest.success) {
      if (domain) {
        await accountManager.saveRenewalLog(
          connection.id,
          domain,
          `SSH connection failed: ${sshTest.error}`,
        );
      }
      await operationManager.updateOperation(operationId, {
        status: "failed",
    ...
    if (domain) {
      await accountManager.saveRenewalLog(
        connection.id,
        domain,
        `SSH connection successful - admin user detected`,
      );
    }
    ...
    await operationManager.updateOperation(operationId, {
      progress: 30,
      message:
        "SSH connection successful. Starting Cisco Tomcat service restart...",
    });
    ...
    if (domain) {
      await accountManager.saveRenewalLog(
        connection.id,
        domain,
        `Executing command: utils service restart Cisco Tomcat`,
      );
    }
    ...
    const restartResult = await SSHClient.executeCommandWithStream({
      ...
      onData: async (chunk: string, totalOutput: string) => {
        ...
        if (domain) {
          if (
            chunk.includes("[STOPPING]") ||
            totalOutput.includes("Cisco Tomcat[STOPPING]")
          ) {
            await accountManager.saveRenewalLog(
              connection.id,
              domain,
              `Detected Cisco Tomcat [STOPPING] - service is shutting down`,
            );
          }
          if (
            chunk.includes("[STARTING]") ||
            totalOutput.includes("Cisco Tomcat[STARTING]")
          ) {
            await accountManager.saveRenewalLog(
              connection.id,
              domain,
              `Detected Cisco Tomcat [STARTING] - service is starting up`,
            );
          ...
          if (
            ...
            totalOutput.includes("Cisco Tomcat[RUNNING]")
          ) {
            await accountManager.saveRenewalLog(
              connection.id,
              domain,
              `Detected Cisco Tomcat [RUNNING] - service is now running`,
            );
          }
        }
      },
      ...
      if (isTimeout) {
        if (domain) {
          await accountManager.saveRenewalLog(
            connection.id,
            domain,
            `Service restart initiated but confirmation timed out. The service is likely still restarting.`,
          );
        }
        await operationManager.updateOperation(operationId, {
          status: "completed",
          progress: 100,
          message:
            "Service restart initiated - confirmation timed out. Manual verification recommended.",
          metadata: {
        ...
        Logger.warn(
          `Service restart confirmation timed out for ${fqdn} - service is likely still restarting`,
        );
      } else {
        if (domain) {
          await accountManager.saveRenewalLog(
            connection.id,
            domain,
            `Service restart failed: ${restartResult.error}`,
          );
        }
        await operationManager.updateOperation(operationId, {
          status: "failed",
        ...
        Logger.error(
          `Failed to restart Cisco Tomcat service for ${fqdn}: ${restartResult.error}`,
        );
      }
    }
  } catch (error: any) {
    if (domain) {
      await accountManager.saveRenewalLog(
        connection.id,
        domain,
        `Internal error during service restart: ${error.message}`,
      );
    }
    await operationManager.updateOperation(operationId, {
      status: "failed",
    ...
    Logger.error(
      `Error during service restart operation ${operationId} for ${fqdn}: ${error.message}`,
    );
  }
}
...
app.post(
  "/api/admin/cancel-renewal/:renewalId",
  asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
      ...
      Logger.info(
        `Admin cancelled renewal ${renewalId} for connection ${operation.connection_id}`,
      );
...
app.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    try {
      ...
      const permissionResults: Record<
        string,
        {
          readable: boolean;
          writable: boolean;
...
httpServer.listen(PORT, "0.0.0.0", async () => {
  ...
  if (certStatus.missing.length > 0) {
    ...
    Logger.info(
      `All root certificates present: ${certStatus.present.join(", ")}`,
    );
  }
