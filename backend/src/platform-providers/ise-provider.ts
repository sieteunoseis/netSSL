...
export class ISEProvider extends PlatformProvider {
    ...
    try {
      ...
      try {
        ...
        if (apiError.message?.includes("409")) {
          Logger.info(
            `CSR conflict on ISE (409), deleting existing CSR and retrying...`,
          );
      ...
      if (
        ...
        if (!csrId) {
          throw new Error(
            "CSR generation succeeded but no CSR ID was returned",
          );
        }

        Logger.info(
          `CSR generated on ISE with ID: ${csrId}, exporting CSR PEM...`,
        );

        // Export the CSR PEM content (ISE doesn't return it inline)
        const csrPem = await this.exportCSR(
          hostname,
          username,
          password,
          csrId,
  ...
  private async deleteConflictingCSR(
    ...
    const pendingCSRs = await this.listPendingCSRs(
      hostname,
      username,
      password,
    );
    for (const csr of pendingCSRs) {
      try {
        Logger.info(
          `Deleting stale CSR on ISE: ${csr.friendlyName} (ID: ${csr.id}, host: ${csr.hostName})`,
        );
  ...
  async exportCSR(
    ...
    if (
      ...
      response.includes("BEGIN CERTIFICATE REQUEST")
    ) {
      return response;
    }
  ...
  > {
    try {
      ...
      Logger.warn(
        `Failed to list pending CSRs on ISE ${hostname}: ${error.message}`,
      );
      return [];
    }
  }
  ...
  async neutraliseConflictingTrustedCerts(
    ...
    try {
      Logger.info(
        `Checking for conflicting trusted certs with CN=${subjectCN} on ${hostname}`,
      );
      ...
      for (const cert of response.response) {
        ...
        if (
          subject.includes(`CN=${subjectCN}`) ||
          friendlyName.includes(subjectCN)
        ) {
          // --- Attempt 1: strip all trust roles via PUT ---
          try {
            Logger.info(
              `Stripping trust roles from conflicting cert: "${friendlyName}" (ID: ${cert.id}, subject: ${subject})`,
            );
            await this.makeApiRequest(
              hostname,
              username,
            ...
            resolvedCount++;
            Logger.info(
              `Successfully stripped trust roles from cert ${cert.id}`,
            );
            continue; // No need to also try DELETE
          } catch (putError: any) {
            Logger.warn(
              `Failed to strip trust roles from cert ${cert.id}: ${putError.message}`,
            );
          }
          ...
          try {
            Logger.info(
              `Attempting to delete conflicting trusted cert: "${friendlyName}" (ID: ${cert.id})`,
            );
            await this.makeApiRequest(
              hostname,
              username,
            ...
            resolvedCount++;
            Logger.info(
              `Successfully deleted conflicting trusted cert: ${cert.id}`,
            );
          } catch (deleteError: any) {
            Logger.warn(
              `Failed to delete conflicting trusted cert ${cert.id}: ${deleteError.message}`,
            );
          }
        }
      }
      ...
      if (resolvedCount > 0) {
        Logger.info(
          `Resolved ${resolvedCount} conflicting trusted cert(s) for CN=${subjectCN}`,
        );
      }
      return resolvedCount;
    } catch (error: any) {
      Logger.warn(
        `Could not check/resolve conflicting trusted certs: ${error.message}`,
      );
      return 0;
    }
  }
  ...
  async bindSignedCertificate(
    ...
    try {
      ...
      Logger.info(
        `Successfully bound certificate to CSR ${csrId} on ${hostname}`,
      );
      return {
        success: true,
        message:
          response?.response?.message || "Certificate was successfully bound",
      };
    } catch (error: any) {
      Logger.error(
        `Failed to bind certificate to CSR ${csrId} on ${hostname}:`,
        error,
      );
      return {
        success: false,
  ...
  async uploadTrustCertificates(
    ...
    try {
      ...
      const existingFingerprints =
        await this.getExistingTrustCertificateFingerprints(
          hostname,
          username,
          password,
        );
      ...
      for (const cert of caCertificates) {
        ...
        if (existingFingerprints.has(certFingerprint)) {
          Logger.info(
            `Certificate already exists on ${hostname}, skipping upload. Fingerprint: ${certFingerprint}`,
          );
          skippedCount++;
          continue;
        }
        ...
        try {
          ...
          Logger.info(
            `Successfully uploaded trust certificate to ${hostname}. Fingerprint: ${certFingerprint}`,
          );
        } catch (error: any) {
          Logger.error(
            `Failed to upload trust certificate to ${hostname}:`,
            error,
          );

          // Provide user-friendly error message
          let userMessage =
            error.message || "Failed to upload trust certificate";
      ...
      Logger.error(
        `Failed to upload trust certificates to ISE ${hostname}:`,
        error,
      );
      return {
        success: false,
  ...
  async getExistingTrustCertificateFingerprints(
    ...
    try {
      Logger.info(
        `Getting existing trust certificate fingerprints from ISE: ${hostname}`,
      );
      ...
      Logger.info(
        `Found ${fingerprints.size} existing trust certificates on ${hostname}`,
      );
      return fingerprints;
    } catch (error: any) {
      Logger.error(
        `Failed to get trust certificate fingerprints from ISE ${hostname}:`,
        error,
      );
    ...
    try {
      ...
      if (
        ...
        for (const resource of response.SearchResult.resources) {
          ...
          if (
            certResponse &&
            certResponse.TrustedCertificate &&
            certResponse.TrustedCertificate.certificateContent
          ) {
            certificates.push(
              certResponse.TrustedCertificate.certificateContent,
            );
      ...
      Logger.error(
        `Failed to get trust certificates from ISE ${hostname}:`,
        error,
      );
      return [];
    }
  ...
  async restartServices(
    ...
    try {
      ...
      const result = await this.executeSSHCommand(
        this.config.sshConfig.serviceRestartCommand,
      );
  ...
  async validateConnection(
    ...
    try {
      ...
      try {
        ...
        Logger.warn(
          `ISE API connection failed for ${hostname}, trying SSH:`,
          apiError,
        );
      }
   ...
   */
  private getCertificateRoles(usage: string | undefined): {
    usedFor: string;
    roles: Record<string, boolean>;
  } {
    const roleMap: Record<
      string,
      { usedFor: string; roles: Record<string, boolean> }
    > = {
      multi_use: {
        usedFor: "MULTI-USE",
        roles: { admin: true, portal: true, eap: true },
      },
  ...
  async prepareCSR(ctx: RenewalContext): Promise<string> {
    ...
    if (!fullFQDN) {
      throw new Error(
        "Invalid connection configuration: missing hostname/domain",
      );
    }

    if (!connection.username || !connection.password) {
      throw new Error(
        "Username and password are required for ISE CSR generation",
      );
    }
    ...
    const saveCSRMetadata = async (csrId: string, csrHostName: string) => {
      ...
      await fs.promises.writeFile(
        metadataPath,
        JSON.stringify({ csrId, hostName: csrHostName }),
      );
    ...
    if (connection.ise_certificate && connection.ise_certificate.trim()) {
      const csrMatch = connection.ise_certificate.match(
        ...
        if (nodes.length > 0) {
          try {
            ...
            if (matchingCSR) {
              ...
              status.logs.push(
                `Found pending CSR on ISE (ID: ${matchingCSR.id}) — will use bind for installation`,
              );
              await ctx.saveLog(
                `Found pending CSR on ISE (ID: ${matchingCSR.id}) — will use bind for installation`,
              );
              ...
              status.logs.push(
                `No matching pending CSR found on ISE — will use import for installation`,
              );
              await ctx.saveLog(
                `No matching pending CSR found on ISE — will use import for installation`,
              );
            ...
            Logger.warn(
              `Could not look up pending CSRs on ISE: ${lookupError.message}`,
            );
            status.logs.push(
              `Could not look up pending CSRs — will use import for installation`,
            );
          }
        }
      }
    ...
    try {
      ...
      if (connection.ise_csr_config) {
        try {
          csrConfig = {
            ...csrConfig,
            ...JSON.parse(connection.ise_csr_config),
          };
        } catch (e) {
          Logger.warn("Invalid ise_csr_config JSON, using defaults");
        }
      }

      // Derive usedFor from certificate usage selection
      const { usedFor } = this.getCertificateRoles(
      ...
      if (!csrResponse.success || !csrResponse.csr) {
        throw new Error(
          csrResponse.message || "Failed to generate CSR from ISE",
        );
      }

      status.logs.push(
        `CSR generated successfully from ISE: ${csrResponse.csr.length} characters`,
      );
      await ctx.saveLog(
        `CSR generated successfully from ISE: ${csrResponse.csr.length} characters`,
      );
      ...
      if (csrResponse.csrId) {
        await saveCSRMetadata(
          csrResponse.csrId,
          csrResponse.hostName || primaryNode,
        );
        status.logs.push(
          `CSR ID saved (${csrResponse.csrId}) — will use bind for installation`,
        );
        await ctx.saveLog(
          `CSR ID saved (${csrResponse.csrId}) — will use bind for installation`,
        );
  ...
  async installCertificate(
    ...
    if (!connection.username || !connection.password) {
      throw new Error(
        "Username and password are required for ISE certificate upload",
      );
    }
    ...
    try {
      ...
      try {
        ...
        const intermediateCert = await fs.promises.readFile(
          intermediatePath,
          "utf8",
        );
      ...
      if (!filesFound) {
        status.logs.push(
          `Certificate files not found, parsing from certificate chain`,
        );
      ...
      if (caCertificates.length > 0) {
        status.logs.push(
          `Uploading ${caCertificates.length} CA certificate(s) to ISE nodes`,
        );
        await ctx.saveLog(
          `Uploading ${caCertificates.length} CA certificate(s) to ISE nodes`,
        );
        ...
        for (const node of nodes) {
          try {
            const caResult = await this.uploadTrustCertificates(
              node,
              connection.username,
              connection.password,
              caCertificates,
            ...
            if (caResult.success) {
              ...
              status.logs.push(
                `CA certificate upload warning for ${node}: ${caResult.message}`,
              );
              await ctx.saveLog(
                `CA certificate upload warning for ${node}: ${caResult.message}`,
              );
            ...
            if (
              ...
              Logger.warn(
                `Connection issue detected for ${node}: ${error.message}`,
              );
            }
          }
        }
      ...
      try {
        ...
        status.logs.push(
          `Certificate file not found, extracting from certificate chain`,
        );
        const certParts = certificate.split("-----END CERTIFICATE-----");
        const certificates = certParts
          .filter((part) => part.includes("-----BEGIN CERTIFICATE-----"))
      ...
      try {
        ...
        /* no CSR metadata — will fall back to import */
      }

      if (csrMetadata && csrMetadata.csrId) {
        status.logs.push(
          `Using CSR bind approach (CSR ID: ${csrMetadata.csrId})`,
        );
        await ctx.saveLog(
          `Using CSR bind approach (CSR ID: ${csrMetadata.csrId})`,
        );
        ...
        try {
          ...
          if (resolvedCount > 0) {
            status.logs.push(
              `Resolved ${resolvedCount} conflicting trusted cert(s) with CN=${fullFQDN}`,
            );
            await ctx.saveLog(
              `Resolved ${resolvedCount} conflicting trusted cert(s) with CN=${fullFQDN}`,
            );
          ...
          status.logs.push(
            `Warning: Could not resolve conflicting trusted certs: ${cleanupError.message}`,
          );
          await ctx.saveLog(
            `Warning: Could not resolve conflicting trusted certs: ${cleanupError.message}`,
          );
        ...
        const { roles: derivedRoles } = this.getCertificateRoles(
          connection.ise_application_subtype,
        );
        let roles: {
          admin?: boolean;
        ...
        if (
          connection.ssl_provider === "letsencrypt" ||
          connection.ssl_provider === "lets_encrypt"
        ) {
          if (roles.pxgrid || roles.ims) {
            const stripped: string[] = [];
            if (roles.pxgrid) {
              roles.pxgrid = false;
              stripped.push("pxGrid");
            }
            if (roles.ims) {
              roles.ims = false;
              stripped.push("IMS");
        ...
        // Admin role triggers a full ISE application server restart which can
        // take 10-20+ minutes. Non-admin roles only reload the specific service
        // and respond within ~60-90 seconds.
        const includesAdmin = roles.admin === true;
        ...
        if (bindResult.success) {
          status.logs.push(
            `Certificate successfully bound to CSR on ${csrMetadata.hostName}`,
          );
          await ctx.saveLog(
            `Certificate successfully bound to CSR on ${csrMetadata.hostName}`,
          );
        ...
        // For admin role, a timeout is expected — ISE restarts the application
        // server and can't respond. The bind request was accepted; the cert will
        // be applied once ISE finishes restarting.
        if (includesAdmin && bindResult.message.includes("timeout")) {
          const msg =
            `Bind request sent — ISE is restarting (admin role triggers a full application server restart). ` +
            `Certificate will be applied when services resume. This typically takes 10-20 minutes.`;
          status.logs.push(msg);
          await ctx.saveLog(msg);

          // Clean up CSR metadata
          try {
            await fs.promises.unlink(path.join(certDir, "csr_metadata.json"));
          } catch {
            /* ignore */
        ...
        // Non-admin bind actually failed — fall through to import
        status.logs.push(
          `CSR bind failed: ${bindResult.message} — falling back to import`,
        );
        await ctx.saveLog(
          `CSR bind failed: ${bindResult.message} — falling back to import`,
        );
      ...
      if (connection.ise_private_key && connection.ise_private_key.trim()) {
        ...
        status.logs.push(
          `Using provided private key for ISE certificate import`,
        );
      } else {
        ...
        try {
          ...
          throw new Error(
            "Private key not found and CSR bind not available. Please ensure private key is provided or generate CSR via ISE/netSSL.",
          );
        }
      }
      ...
      if (connection.ise_cert_import_config) {
        try {
          ...
          status.logs.push(
            `Warning: Invalid JSON in ISE import config, using defaults`,
          );
        }
      }
      ...
      status.logs.push(
        `Certificate import completed: ${successCount}/${totalCount} nodes successful`,
      );
      await ctx.saveLog(
        `Certificate import completed: ${successCount}/${totalCount} nodes successful`,
      );
      ...
      for (const nodeResult of result.results) {
        if (nodeResult.status === "success") {
          ...
          if (
            nodeResult.message &&
            (nodeResult.message.includes("SSL certificate") ||
              nodeResult.message.includes("expired"))
          ) {
      ...
      if (successCount === 0) {
        ...
        if (
          firstError.includes("SSL certificate") ||
          firstError.includes("expired")
        ) {
          throw new Error(
            `Failed to import certificate to any ISE nodes. All nodes have SSL certificate issues. Please update the SSL certificates on your ISE servers before proceeding.`,
          );
        } else if (firstError.includes("Connection failed")) {
          throw new Error(
            `Failed to import certificate to any ISE nodes. Connection issues detected. Please verify ISE hostnames, network connectivity, and that ISE services are running.`,
          );
        } else {
          throw new Error(
            `Failed to import certificate to any ISE nodes. Error: ${firstError}`,
          );
        }
      }
    ...
    for (const node of nodes) {
      ...
      try {
        ...
        if (error.message && error.message.includes("expired")) {
          ...
          error.message &&
          error.message.includes("API request failed: 401")
        ) {
          ...
          error.message &&
          error.message.includes("API request failed: 409")
        ) {
