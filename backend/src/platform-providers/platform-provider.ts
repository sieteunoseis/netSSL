...
export abstract class PlatformProvider {
  ...
  protected async makeApiRequest(
    ...
    try {
      Logger.info(
        `Making API request to ${fullUrl} with username: ${username}`,
      );
      ...
      if (method !== "GET") {
        try {
          ...
          if (csrf?.token) {
            ...
            Logger.info(
              `CSRF token and session cookie obtained for ${hostname}`,
            );
          } else {
            Logger.warn(
              `No CSRF token returned from ${hostname} — proceeding without it`,
            );
          }
        } catch (csrfError: any) {
          Logger.warn(
            `Could not fetch CSRF token for ${hostname}: ${csrfError.message} - proceeding without it`,
          );
        }
      }
      ...
      if (!response.ok) {
        ...
        Logger.error(
          `API ${method} ${fullUrl} failed: ${response.status}, auth user: ${username}, response: ${response.body?.substring(0, 500)}`,
        );
      ...
      if (error.cause) {
        ...
        if (cause.code === "CERT_HAS_EXPIRED") {
          throw new Error(
            `Connection failed: The SSL certificate on ${hostname} has expired. Please update the certificate on the ISE server before proceeding.`,
          );
        }

        if (cause.code === "UNABLE_TO_VERIFY_LEAF_SIGNATURE") {
          throw new Error(
            `Connection failed: Unable to verify SSL certificate on ${hostname}. This may be due to a self-signed certificate or certificate chain issue.`,
          );
        }

        if (cause.code === "ECONNREFUSED") {
          throw new Error(
            `Connection failed: Unable to connect to ${hostname}. Please verify the hostname and ensure the ISE service is running.`,
          );
        }

        if (cause.code === "ENOTFOUND") {
          throw new Error(
            `Connection failed: Hostname ${hostname} could not be resolved. Please verify the hostname is correct and DNS is configured properly.`,
          );
        }

        if (cause.code === "ETIMEDOUT") {
          throw new Error(
            `Connection failed: Connection to ${hostname} timed out. Please verify the hostname and network connectivity.`,
          );
        }
      }
      ...
      if (error.message === "fetch failed") {
        throw new Error(
          `Connection failed: Unable to connect to ${hostname}. Please verify the hostname, network connectivity, and that the ISE service is running.`,
        );
      }
  ...
  private async fetchCSRFToken(
    ...
    return new Promise((resolve) => {
      const req = https.request(
        ...
        (res: any) => {
          ...
          res.on("end", () => {
            ...
            const setCookies = res.headers["set-cookie"] as
              | string[]
              | undefined;
            ...
            const headerKeys = Object.keys(res.headers).filter(
              (k: string) =>
                k.includes("csrf") ||
                k.includes("cookie") ||
                k.includes("token"),
            ...
            Logger.info(
              `CSRF fetch from ${hostname}: status=${res.statusCode}, token=${csrfToken ? "present" : "missing"}, cookies=${cookieStr ? "present" : "none"}, relevant-headers=[${headerKeys.join(",")}]`,
            );
            if (csrfToken) {
      ...
      req.on("error", (err: any) => {
        Logger.error(`CSRF fetch error for ${hostname}: ${err.message}`);
        resolve(null);
      });
      req.on("timeout", () => {
        Logger.warn(`CSRF fetch timeout for ${hostname}`);
        req.destroy();
        resolve(null);
      });
      req.end();
  ...
  protected async connectSSH(
    ...
    Logger.info(
      `Connecting to SSH with algorithms:`,
      this.config.sshConfig.connectionAlgorithms,
    );
