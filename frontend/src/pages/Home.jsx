...
import {
  filterEnabledConnections,
  getConnectionDisplayHostname,
  getIseMonitoredHostname,
  isConnectionEnabled,
...
const Home = ({ onStatusUpdate }) => {
  ...
  const [viewMode, setViewMode] = useState(
    () => localStorage.getItem("netssl-viewMode") || "full",
  );
  const [searchTerm, setSearchTerm] = useState("");
  const [statusFilter, setStatusFilter] = useState("all");
  const [typeFilter, setTypeFilter] = useState("all");
  const [sortBy, setSortBy] = useState(
    () => localStorage.getItem("netssl-sortBy") || "name",
  );
  const [sortOrder, setSortOrder] = useState(
    () => localStorage.getItem("netssl-sortOrder") || "asc",
  );
  ...
  const fetchConnections = async (retryCount = 0) => {
    try {
      ...
      iseConnections.forEach((conn) => {
        console.log(
          `${conn.name}: app_type=${conn.application_type}, enable_ssh=${conn.enable_ssh}`,
        );
      });
  ...
  const fetchCertificateStatus = async (connection) => {
    try {
      ...
      console.error(
        `Error fetching certificate status for ${connection.name}:`,
        error,
      );
      setCertificateStatuses((prev) => ({
        ...prev,
        [connection.id]: { error: error.message },
      }));
    }
  };
    ...
    try {
      ...
      if (response.ok && result.success) {
        toast({
          ...
          description:
            result.message ||
            `Successfully connected to ${connection.name} via SSH`,
          duration: 5000,
        });
      } else {
      ...
      toast({
        ...
        description:
          error.message ||
          `Failed to connect to ${connection.name} via SSH. Check credentials and connectivity.`,
        variant: "destructive",
        duration: 5000,
      });
    } finally {
  ...
  const getCertificateStatus = (connection) => {
    ...
    const portSuffix =
      portChecks && portChecks.length > 1
        ? ` (${portChecks.filter((p) => p.success).length}/${portChecks.length} ports)`
        : "";
    const somePortsFailed =
      portChecks && portChecks.length > 1 && portChecks.some((p) => !p.success);
  ...
  const overallStatus = useMemo(() => {
    ...
    if (enabledConnections.length === 0)
      return { total: 0, valid: 0, expiring: 0, expired: 0, autoRenew: 0 };

    const summary = enabledConnections.reduce(
      (acc, conn) => {
    ...
    connectionState.connections,
    certificateStatuses,
    certificateSettings.warningDays,
  ]);
  ...
  useEffect(() => {
    localStorage.setItem("netssl-viewMode", viewMode);
  }, [viewMode]);
  useEffect(() => {
    localStorage.setItem("netssl-sortBy", sortBy);
  }, [sortBy]);
  useEffect(() => {
    localStorage.setItem("netssl-sortOrder", sortOrder);
  }, [sortOrder]);
  ...
  const filteredConnections = useMemo(() => {
    ...
    let filtered = connectionState.connections.filter((conn) => {
      ...
      const matchesType =
        typeFilter === "all" || conn.application_type === typeFilter;

      const status = getCertificateStatus(conn);
      let matchesStatus = true;
      if (statusFilter === "valid") matchesStatus = status.status === "valid";
      if (statusFilter === "expiring")
        matchesStatus = status.status === "expiring";
      if (statusFilter === "expired")
        matchesStatus = status.status === "expired";
  ...
  const getCertGrade = (certInfo) => {
    if (!certInfo || certInfo.error)
      return { grade: "?", color: "text-muted-foreground", bg: "bg-muted" };
    let score = 100;
    ...
    if (score >= 40)
      return { grade: "D", color: "text-orange-500", bg: "bg-orange-500/10" };
    return {
      grade: "F",
  ...
  const renderConnectionCard = (connection) => {
    ...
    return (
      <Card
        key={connection.id}
        className={`overflow-hidden border-l-2 ${status.status === "valid" ? "border-l-status-valid" : status.status === "expiring" || status.status === "mismatch" ? "border-l-status-warning" : status.status === "expired" ? "border-l-status-expired" : "border-l-border"} ${!isEnabled ? "opacity-40 grayscale border-dashed" : ""}`}
      >
        <Collapsible
          open={isExpanded}
          onOpenChange={() => toggleCardExpansion(connection.id)}
        >
                  ...
                  <div className="flex items-center gap-4 flex-1">
                    <div className="flex-1 text-left">
                      <div className="flex items-center gap-3 mb-1">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {connection.auto_renew &&
                            connection.dns_provider !== "custom" && (
                              <Badge
                                variant="secondary"
                                className="text-xs px-2 py-1 "
                              >
                      ...
                      <div className="flex flex-col text-sm text-muted-foreground">
                        ...
                        {getIseMonitoredHostname(connection) &&
                          getIseMonitoredHostname(connection) !==
                            getConnectionDisplayHostname(connection) && (
                        ...
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <Button
                              ...
                              onClick={(e) => {
                                e.preventDefault();
                                handleToggleConnection(connection);
                              }}
                            >
                              {isEnabled ? (
                                <ToggleRight className="h-3.5 w-3.5 text-status-valid" />
                              ) : (
                        ...
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <Button
                              ...
                              onClick={(e) => {
                                e.preventDefault();
                                navigate(`/logs?connection=${connection.id}`);
                              }}
                            >
                          ...
                          <TooltipTrigger asChild>
                            <Button
                              ...
                              onClick={(e) => {
                                e.preventDefault();
                                window.open(
                                  `https://${getConnectionDisplayHostname(connection)}`,
                                  "_blank",
                                );
                              }}
                            >
              ...
              {(() => {
                ...
                return (
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <div
                        className={`flex items-center justify-center w-7 h-7 rounded-md font-bold text-sm ${gradeInfo.bg} ${gradeInfo.color}`}
                      >
                        {gradeInfo.grade}
                      </div>
                    </TooltipTrigger>
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <div className="flex items-center gap-1">
                    ...
                    <span>
                      {certInfo.keyAlgorithm || "RSA"} {certInfo.keySize || "?"}
                    </span>
                  </div>
                </TooltipTrigger>
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <div className="flex items-center gap-1">
                        ...
                        <span>
                          {certInfo.subjectAltNames.length} SAN
                          {certInfo.subjectAltNames.length !== 1 ? "s" : ""}
                        </span>
                      </div>
                    </TooltipTrigger>
                    <TooltipContent>
                      {certInfo.subjectAltNames.join(", ")}
                    </TooltipContent>
                  </Tooltip>
                )}
              {certInfo.timings && (
  ...
  if (connectionState.isRetrying) {
    return (
      <>
        ...
        <LoadingState
          variant="page"
          text={`Connecting to server... (attempt ${connectionState.retryAttempt} of 10)`}
        />
      </>
    );
  }
        ...
        <div className="mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-6">
            ...
            <Card className="border-l-2 border-l-status-valid animate-slide-up animate-stagger-2">
              ...
              <CardContent>
                <div className="text-2xl font-bold font-mono text-status-valid">
                  {overallStatus.valid}
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="border-l-2 border-l-status-expired animate-slide-up animate-stagger-4">
              ...
              <CardContent>
                <div className="text-2xl font-bold font-mono text-status-expired">
        ...
        <div className="mb-4 flex items-center justify-between flex-wrap gap-2">
          <div className="flex items-center gap-2 flex-wrap">
            <p className="text-sm text-muted-foreground">
              ...
              of {connectionState.connections.length} connections
            </p>
            {searchTerm && (
              <Badge
                variant="outline"
                      ...
                      return (
                        <>
                          ...
                          >
                            ...
                            <td className="p-4 font-mono text-sm max-w-[200px] truncate">
                              {getConnectionDisplayHostname(connection)}
                              {getIseMonitoredHostname(connection) &&
                                getIseMonitoredHostname(connection) !==
                                  getConnectionDisplayHostname(connection) && (
                            ...
                            <td className="p-4">
                              {(() => {
                                ...
                                if (status.status === "expired")
                                  return (
                                    <Ban className="h-4 w-4 text-status-expired" />
                                  );
