...
const CSRGeneratorWizard = ({
  ...
  const [formData, setFormData] = useState({
    ...
    organization: "",
    organizationalUnit: "",
    keySize: keySizeOverride || "2048",
  });
  const [generatedData, setGeneratedData] = useState(null);

  useEffect(() => {
    if (keySizeOverride) {
      setFormData((prev) => ({ ...prev, keySize: keySizeOverride }));
    }
  }, [keySizeOverride]);

  // Reset wizard state whenever the dialog closes so reopening doesn't leak stale entries.
  useEffect(() => {
    if (!isOpen) {
      setCurrentStep(0);
      setFormData({
      ...
      fields: [
        {
          ...
          type: "radio",
          description:
            "2048-bit is recommended for most use cases. 4096-bit provides higher security but slower performance.",
          required: true,
          options: [
            {
              value: "2048",
              label: "2048-bit (Recommended)",
              description: "Standard security, faster performance",
            },
            {
              value: "4096",
              label: "4096-bit (High Security)",
              description: "Enhanced security, slower performance",
            },
          ],
        },
      ],
  ...
  const validateStep = (stepIndex) => {
    ...
    return requiredFields.every((field) => {
      ...
      if (field.key === "country") {
        return (
          value && value.length === 2 && /^[A-Z]{2}$/.test(value.toUpperCase())
        );
      }
  ...
  const handleDialogKeyDown = (e) => {
    ...
    if (
      (e.key === "Enter" || e.key === " ") &&
      activeElement?.hasAttribute("data-next-button")
    ) {
      e.preventDefault();
      handleNext();
      return;
  ...
  const handleGenerate = async () => {
    ...
    try {
      // Build common name — prop override wins (used by ISE local mode for custom CN)
      const commonName =
        commonNameOverride ||
        (hostname && domain
          ? `${hostname}.${domain}`
                ...
                <CardContent className="space-y-4">
                  ...
                  {currentStep === 0 && (hostname || domain) && (
                    <Alert>
                      ...
                      <AlertDescription>
                        <strong>Common Name will be:</strong>{" "}
                        {hostname && domain
                          ? `${hostname}.${domain}`
              ...
              <CardContent className="space-y-4">
                ...
                <Alert>
                  ...
                  <AlertDescription>
                    {mode === "configure" ? (
                      <>
                        <strong>Configured!</strong> These subject details will
                        be sent to the ISE API during certificate renewal. ISE
                        will generate the CSR and hold the private key
                        internally.
                      </>
                    ) : (
                      <>
                        <strong>Success!</strong> Your Certificate Signing
                        Request and private key have been generated using
                        industry-standard cryptographic libraries. The CSR is
                        ready to be submitted to your Certificate Authority.
                      </>
                    )}
                  </AlertDescription>
                </Alert>
